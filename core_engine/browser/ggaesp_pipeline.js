...
function toSignal01(n, fallback = 0.5) {
    return clamp(typeof n === "number" ? n : fallback, 0, 100) / 100;
}
function sourceQualityWeight(src) {
 ...
 */
function runV11(input) {
    const values = Object.values(input.data).filter((v) => typeof v === "number");
    ...
    const falseCorrelation = typeof input.data.falseCorrelation === "number" ? clamp(input.data.falseCorrelation) : 20;
    const baseScore = clamp((structure + growth + analysis + movement + stability + profit) / 6 - ((risk + noise + falseCorrelation) / 3) * 0.35);
    return {
        structure,
        growth,
        analysis,
...
function runV13(v12, input) {
    ...
    const maybeSources = Array.isArray(input.data.collectiveSources) ? input.data.collectiveSources : [];
    ...
    if (maybeSources.length > 0) {
        ...
        const weightedVariance = weighted.reduce((acc, x) => {
...
function runV15(v14, branch) {
    ...
    const irreversibleRiskPenalty = clamp(Math.max(0, v14.risk - 60) * 0.45 + Math.max(0, v14.noise - 60) * 0.35 + v14.emotionalBiasPenalty * 0.8, 0, 40);
    ...
    const guardianIntegrityScore = clamp(Math.round(70 + (nearMissFlag ? 12 : 0) + triggerCount * 4 - Math.max(0, irreversibleRiskPenalty - 25) * 0.5), 40, 98);
    if (nearMissFlag) {
        guardianNotes.push("Guardian: near-miss prevented (risk escalation intercepted).");
    }
    guardianNotes.push(`Guardian: safetyFactor=${guardianSafetyFactor.toFixed(2)} irreversibleRiskPenalty=${Math.round(irreversibleRiskPenalty)} GIS=${guardianIntegrityScore}`);
    return {
        ...v14,
        score,
        decision,
 ...
 */
function runV16(v15, input, branch) {
    const memoryCapsule = {
 ...
 */
function runV19(v18, input, energyMode, branch) {
 ...
 * MAIN PIPELINE
 */
export function runGGAESP(input) {
    const safeInput = {
        ...input,
        timestamp: input.timestamp ?? new Date().toISOString(),
        data: input.data ?? {},
    ...
    const branch = {
        ...
        created_at: safeInput.timestamp,
    };
