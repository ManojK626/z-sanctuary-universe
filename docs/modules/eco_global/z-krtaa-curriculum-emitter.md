...
| **Safety class** | low |
| **Z registry ZID** | z-krtaa-curriculum-emitter |
| **Zuno audit status** | FOUND_PARTIAL |

## Purpose

Read-only ingest of hub reports; emits advisory curriculum JSON + Markdown (npm run krtaa:curriculum-emit). Dashboard: dashboard/z-ai-ecosphere. Prompt packs v3: docs/z-krtaa-prompt-packs/. HODP links in Control Centre + Blueprint.
...
Registry lists **doctrine_only**. No `expected_paths` in master registry — treat as doctrine, stub, or external until paths are added with on-disk proof.
