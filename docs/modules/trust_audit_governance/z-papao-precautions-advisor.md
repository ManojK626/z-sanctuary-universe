# Z-PAPAO (Pre-Alert & Precautions Advisor)

## Registry Identity

| Field | Value |
| --------------------- | ----------------------------- |
| **ID** | `z-papao-precautions-advisor` |
| **Category** | trust_audit_governance |
| **Registry status** | planned_stub |
| **Safety class** | low |
| **Z registry ZID** | z-papao-precautions-advisor |
| **Zuno audit status** | FOUND_PARTIAL |

## Purpose

Read-only pre-alert brief from hub reports + registry paths. npm run papao:precaution-emit → data/reports/z_papao_precaution_brief.{json,md}. Dashboard: z-ai-ecosphere.
