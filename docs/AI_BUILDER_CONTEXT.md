...
**Z-ECO-MAP topology (Phase 0):** [Z_ECO_MAP_TOPOLOGY_ENGINE.md](Z_ECO_MAP_TOPOLOGY_ENGINE.md) + `data/z_eco_map_topology_registry.json` — ecosystem knowledge map (relationships, deploy/backup/gateway zones, fusion awareness). **observe → classify → suggest** only; no runtime, AI execution, or deploy mutation.
