...
| [PHASE_E2_Z_LOCAL_GATEWAY_DIRECTORY.md](PHASE_E2_Z_LOCAL_GATEWAY_DIRECTORY.md) | E2 local gateway — registry + `npm run z:local:gateway:status` → `data/reports/z_local_gateway_status.json`; read-only panel `dashboard/panels/z-local-gateway-readonly.html` (**no auto-launch**). Roulette display: `http://127.0.0.1:5190/`. |
