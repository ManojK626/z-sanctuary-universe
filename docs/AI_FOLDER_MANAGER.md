...
5. **SLO and readiness** - keep your forced fail verification in the ZSanctuary_Universe workspace (use
   npm run slo:guard + workspace:auto-sweep). Record the artifact names in AI_FOLDER_MANAGER.md so you can cross-check in the future.
...
- Execute
  npm run html:open-cores from C:\ZSanctuary_Universe to open every GUI link that backs the formulas, dashboards, and panels; treat the browser as a proof of the link map.
